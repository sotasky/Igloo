...
func TestAndroidSyncSubtitleAssetPrefersManualTrack(t *testing.T) {
	...
	mustWriteFile(t, filepath.Join(dataDir, "videos", "youtube", "manual.info.json"), []byte(`{"language":"tr","subtitles":{"en":[{"url":"https://www.youtube.com/api/timedtext?v=sample&caps=asr&lang=en&fmt=vtt"}],"tr":[{"url":"https://example.test/manual.vtt"}]},"automatic_captions":{"en":[{"url":"https://example.test/auto.vtt?kind=asr&lang=en"}]}}`))
	if err := srv.db.ExecRaw(`
		INSERT INTO videos (
			video_id, channel_id, title, duration, thumbnail_path, file_path,
			file_size, media_kind, published_at, sync_seq
