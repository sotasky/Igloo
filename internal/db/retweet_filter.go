...
//     its content_hash is muted. Joins channel_settings via the
//     deterministic 'twitter_<lower(handle)>' channel id format.
...
func retweetFilterClause(alias string) string {
	return fmt.Sprintf(`NOT (
		...
		AND COALESCE(%[1]s.source_handle,'') != ''
		AND lower(%[1]s.source_handle) != lower(COALESCE(%[1]s.author_handle,''))
		AND EXISTS (
			SELECT 1 FROM channel_settings cs
			WHERE cs.channel_id = 'twitter_' || lower(%[1]s.source_handle)
			  AND cs.include_reposts = 0
		)
		AND NOT EXISTS (
			SELECT 1 FROM retweet_sources rs
			LEFT JOIN channel_settings cs2 ON cs2.channel_id = 'twitter_' || lower(rs.retweeter_handle)
	...
	AND NOT (
		...
		AND COALESCE(%[1]s.author_handle,'') != ''
		AND lower(%[1]s.author_handle) != lower(COALESCE(%[1]s.quote_author_handle,''))
		AND EXISTS (
			SELECT 1 FROM channel_settings cs
			WHERE cs.channel_id = 'twitter_' || lower(%[1]s.author_handle)
			  AND cs.include_reposts = 0
		)
	)`, alias)
}
