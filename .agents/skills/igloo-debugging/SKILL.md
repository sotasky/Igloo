...
- Android CI runs from `android/` with `./gradlew :app:testDevtestUnitTest`; this is not identical to `android/test.sh`.
...
- Prefer repo scripts (`android/build.sh`, `android/test.sh`, `scripts/dev/build.sh android`, `scripts/dev/build.sh all`) unless reproducing a narrow CI Gradle lane.
...
- Use `scripts/dev/i18n_check.sh` as the durable gate for shared catalog and Android resource generation changes.
- If generated files are stale after templ or catalog edits, regenerate first, then judge the source diff and focused tests.
- If Go tests fail because the default build cache is not writable, retry the same narrow lane with a writable `GOCACHE`.
