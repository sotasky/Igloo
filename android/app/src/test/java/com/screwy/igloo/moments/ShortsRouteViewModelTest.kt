...
class ShortsRouteViewModelTest {
    ...
    @Test fun storyTrayPlaylistContinuesAcrossChannels() = runBlocking {
        ...
        val sub = subscribe(vm)
        val ok = withTimeoutOrNull(2_000L) {
            while (vm.items.value.size < 3 || vm.startIndex.value != 1) delay(10)
            true
        }
        sub.cancel()

        assertEquals(true, ok)
        assertEquals(listOf("v_newer_first", "v_newer_last", "v_older"), vm.items.value.map { it.videoId })
